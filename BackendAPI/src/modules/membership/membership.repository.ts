...
export class MembershipRepository {
  ...
  async addMembershipSeeder() {
    ...
    for (const element of data) {
      const memberships = dbMemberships.find(
        (membership) => membership.name === element.name,
      );
  ...
  async addUserMembership(userId: string, membershipId: string) {
    ...
    if (!user)
      throw new NotFoundException(
        'ID de usuario inválido, intenta con un ID válido.',
      );
    ...
    if (!membership)
      throw new NotFoundException(
        'ID de membresía inválido, intenta con un ID válido.',
      );
  ...
  async findAll() {
    ...
    if (!membership)
      throw new NotFoundException('No existen membresías registradas.');
  ...
  async cancelMembership(userId: string, membershipId: string) {
    ...
    if (!userMembership)
      throw new NotFoundException(
        'No se encontró una membresía activa para el usuario.',
      );
  ...
  async purchaseMembership(membershipId: string) {
    if (!membershipId) {
      throw new NotFoundException(
        'No se encontro la membresia asociada con el ID',
      );
    }
